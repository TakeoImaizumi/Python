...
# ---------------------------------------------------------------------
# --- Plotting and saving Fig.2 (Φ_H vs κ : Discretization comparison) ---
# ---------------------------------------------------------------------
def plot_and_save_fig2():
    """
    Generate Fig2.pdf: Comparison of linear self-coefficients Φ_H
    for several discretization methods vs κ.
    ...
    with mpl.rc_context({
        ...
        # Reference lines
        ax.axvline(1.0, color='gray',  linestyle='--', linewidth=1.0)  # κ=1 (no label)
        ax.axhline(0.0, color='black', linestyle='-',  linewidth=1.0)  # Φ=0 (solid thin black)

        # Labels / limits / grid
        ax.set_xlabel(r'$\kappa$') # <<< MODIFIED HERE
...
patch1 = mpatches.Patch(facecolor="white", edgecolor="black",
                        label="1 (Monostable)") # <<< MODIFIED HERE
...
ax1.text(bistable_start / 2, text_ypos_fig9, "Monostable", ha='center', va='bottom', fontsize=text_fontsize_fig9) # <<< MODIFIED HERE
ax1.text((bistable_start + bistable_end) / 2, text_ypos_fig9, "Bistable", ha='center', va='bottom', fontsize=text_fontsize_fig9)
ax1.text((bistable_end + T_total) / 2, text_ypos_fig9, "Monostable", ha='center', va='bottom', fontsize=text_fontsize_fig9) # <<< MODIFIED HERE
