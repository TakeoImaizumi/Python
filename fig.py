...
#This program is a Python script that reproduces Figures 2 through 8 from the paper "A dynamical model of subjectivity integrating affective gain, cognitive bias, and self-regulation".
#
...
ms_pos_fig3 = generate_trajectory(G_gt1, mu_pos, T_plot, seed_value)
ms_neg_fig3 = generate_trajectory(G_gt1, mu_neg, T_plot, seed_value)
ms_pos_fig4 = generate_trajectory(G_lt1, mu_pos, T_plot, seed_value)
ms_neg_fig4 = generate_trajectory(G_lt1, mu_neg, T_plot, seed_value)
ms_pos_fig5 = generate_trajectory(G_neg, mu_pos, T_plot, seed_value)
ms_neg_fig5 = generate_trajectory(G_neg, mu_neg, T_plot, seed_value)

# Stable points & distributions (Fig.5–6)
...
# ---------------------------------------------------------------------
# --- Plotting and saving Fig.2 (Phase Diagram) -----------------------
# ---------------------------------------------------------------------
...
# Figure markers (Fig.3–5,7 sampling points)
points = {
    "Fig. 3": [(1.5,  0.5), (1.5, -0.5)],
    "Fig. 4": [(0.3,  0.5), (0.3, -0.5)],
    "Fig. 5": [(-0.5, 0.5), (-0.5, -0.5)],
    "Fig. 7": [(1.0,  0.0)],
}
markers = {"Fig. 3": "s", "Fig. 4": "^", "Fig. 5": "o", "Fig. 7": "D"}
for key, coords in points.items():
    face = "white" if key == "Fig. 5" else "none"
    for gx, mx in coords:
        ...
        if key == "Fig. 5": dx, dy = (-0.26, 0.09 if mx > 0 else -0.21)
        if key == "Fig. 3": dx = -0.1
        if key == "Fig. 7": dx, dy = (0.08, 0.08)
...
plt.close()
# ---------------------------------------------------------------------
# --- Plotting and saving Fig.3 (Trajectories for G > 1)  
# ---------------------------------------------------------------------
...
axes[0].plot(x_display, ms_pos_fig3, color="black")
...
axes[1].plot(x_display, ms_neg_fig3, color="black")
...
# --- Plotting and saving Fig.4 (Trajectories for 0 < G < 1) ---
fig, axes = plt.subplots(1, 2, figsize=subplot_figsize, sharey=True)
...
# --- Plotting and saving Fig.5 (Trajectories for G < 0 - Bistable) ---
fig, axes = plt.subplots(1, 2, figsize=subplot_figsize, sharey=True)
...
# --- Plotting and saving Fig.6 (Distributions for G < 0 - Bistable) ---
fig, axes = plt.subplots(1, 2, figsize=subplot_figsize, sharey=True)
...
# --- Plotting and saving Fig.7 (Satori Trajectory, G=1, mu=0) ---
...
# --- Plotting and saving Fig.8 (Trajectory with time-varying G and mu) ---
...
text_fontsize_fig8 = 8
ymax_ax1 = ax1.get_ylim()[1]
offset_fig8 = 0.08 * (ymax_ax1 - Ms_lower)
text_ypos_fig8 = ymax_ax1 + offset_fig8
ax1.text(bistable_start / 2, text_ypos_fig8, "Single Stable", ha='center', va='bottom', fontsize=text_fontsize_fig8)
ax1.text((bistable_start + bistable_end) / 2, text_ypos_fig8, "Bistable", ha='center', va='bottom', fontsize=text_fontsize_fig8)
ax1.text((bistable_end + T_total) / 2, text_ypos_fig8, "Single Stable", ha='center', va='bottom', fontsize=text_fontsize_fig8)
